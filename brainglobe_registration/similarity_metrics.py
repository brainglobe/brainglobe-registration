...
def normalized_cross_correlation(image1: npt.NDArray, image2: npt.NDArray) -> float:
    """
...
def mutual_information(image1: npt.NDArray, image2: npt.NDArray, bins: int = 32) -> float:
    """
...
def structural_similarity_index(image1: npt.NDArray, image2: npt.NDArray) -> float:
    """
...
# Need to fix, can't get it to work yet always returns 0 
#def elastix_metric(image1: npt.NDArray, image2: npt.NDArray) -> float:
#    """
#    Calculate similarity using SimpleITK's MattesMutualInformation metric,
#    similar to what would be used in a real registration workflow.
#    
#    Parameters
#    ----------
#    image1 : npt.NDArray
#        First input image (fixed image)
#    image2 : npt.NDArray
#        Second input image (moving image)
#        
#    Returns
#    -------
#    float
...
#    fixed_image = sitk.GetImageFromArray(image1.astype(np.float32))
#    moving_image = sitk.GetImageFromArray(image2.astype(np.float32))
#    
#    # Set up a basic registration method - we only care about the metric value
#    registration_method = sitk.ImageRegistrationMethod()
#    
#    # Set up the metric just like in the registration workflow
...
#    
#    # We'll use identity transform since we just want the metric value
#    transform = sitk.Transform(2, sitk.sitkIdentity)
#    registration_method.SetInitialTransform(transform)
#    
#    # Disable optimization - we just want the metric value
#    registration_method.SetOptimizerAsExhaustive([0, 0])
#    registration_method.SetOptimizerScales([1, 1])
#    
#    # Get the metric value without running full optimization
#
#        # This will run a single iteration to compute the metric
#    registration_method.SetNumberOfThreads(1)
#    _ = registration_method.MetricEvaluate(fixed_image, moving_image)
#    metric_value = registration_method.GetMetricValue()
#        
#        # Negate since MI is a cost function (lower is better) but we want higher = better
#    return -metric_value
