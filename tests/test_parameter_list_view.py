...
def test_delete_value_in_last_row_without_param_name_appends_new_row(
    parameter_list_view, qtbot
):
    """Test corner case: deleting value in last row appends new row."""
    qtbot.addWidget(parameter_list_view)

    current_row_count = parameter_list_view.rowCount()
    last_row_index = len(param_dict)

    # Set a value in the last row without setting parameter name
    parameter_list_view.setItem(
        last_row_index, 1, QTableWidgetItem("test_value")
    )
    qtbot.wait(100)

    # Now delete the value - should delete row but append new empty row
    with qtbot.waitSignal(parameter_list_view.cellChanged, timeout=1000):
        parameter_list_view.item(last_row_index, 1).setText("")

    # Row count should remain the same (row deleted, new one appended)
    assert parameter_list_view.rowCount() == current_row_count
...
def test_delete_row_when_both_name_and_value_empty(parameter_list_view, qtbot):
    """Test that row is removed when both name and value are empty."""
    qtbot.addWidget(parameter_list_view)

    # Get initial state
    initial_row_count = parameter_list_view.rowCount()
    target_row = 2  # Pick a row index (CheckNumberOfSamples)
    param_item = parameter_list_view.item(target_row, 0)
    param_name = param_item.text() if param_item else None
...
def test_parameter_name_cleared_but_value_remains(parameter_list_view, qtbot):
    """Test clearing name when value exists clears value and deletes."""
    qtbot.addWidget(parameter_list_view)

    initial_row_count = parameter_list_view.rowCount()
    target_row = 2  # Pick a row index (BSplineInterpolationOrder)
    param_item = parameter_list_view.item(target_row, 0)
    param_name = param_item.text() if param_item else None
    assert param_name is not None
...
def test_parameter_renaming(parameter_list_view, qtbot):
    """Test that renaming a parameter updates the dictionary correctly."""
    qtbot.addWidget(parameter_list_view)

    target_row = 4  # Pick a row index (Transform)
    param_item = parameter_list_view.item(target_row, 0)
    old_name = param_item.text() if param_item else None
...
def test_delete_multiple_rows(parameter_list_view, qtbot):
    """Test deleting multiple rows in sequence."""
    qtbot.addWidget(parameter_list_view)

    initial_row_count = parameter_list_view.rowCount()
    # Pick row indices to delete (fetch names dynamically to handle shifting)
    rows_to_delete = [0, 3]  # AutomaticScalesEstimation, CheckNumberOfSamples

    # Delete rows by picking index first, then fetching name
    for target_row in rows_to_delete:
        # Fetch name and value from the row index
        param_item = parameter_list_view.item(target_row, 0)
        param_name = param_item.text() if param_item else None
        if param_name:
            # Clear value first
    ...
    # Verify exactly 2 rows were removed
    final_row_count = parameter_list_view.rowCount()
    assert final_row_count == initial_row_count - 2


def test_clear_value_removes_parameter(parameter_list_view, qtbot):
...
def test_old_param_names_tracking(parameter_list_view, qtbot):
...
def test_set_data_resets_tracking(parameter_list_view, qtbot):
...
def test_delete_last_row_does_not_crash(parameter_list_view, qtbot):
...
def test_empty_value_with_valid_name(parameter_list_view, qtbot):
    """Test behavior when value is empty but name is valid."""
    qtbot.addWidget(parameter_list_view)

    target_row = 1  # Pick a row index (AutomaticTransformInitialization)
    param_item = parameter_list_view.item(target_row, 0)
    param_name = param_item.text() if param_item else None
    assert param_name is not None
...
def test_whitespace_only_parameter_name(parameter_list_view, qtbot):
...
def test_delete_row_when_value_cleared_first(parameter_list_view, qtbot):
    """Test that row is deleted when value is cleared first, then name."""
    qtbot.addWidget(parameter_list_view)

    initial_row_count = parameter_list_view.rowCount()
    target_row = 3  # Pick a row index (CheckNumberOfSamples)
    param_item = parameter_list_view.item(target_row, 0)
    param_name = param_item.text() if param_item else None
    assert param_name is not None
    assert param_name in parameter_list_view.param_dict
...
def test_delete_row_when_both_cleared_via_value(parameter_list_view, qtbot):
...
def test_signals_blocked_in_item_changed(parameter_list_view, qtbot):
...
def test_signals_blocked_in_cell_change(parameter_list_view, qtbot):
...
def test_row_deletion_with_item_without_userrole(parameter_list_view, qtbot):
...
def test_delete_row_via_value_when_both_empty(parameter_list_view, qtbot):
...
def test_row_deletion_else_branches_no_userrole(parameter_list_view, qtbot):
...
def test_delete_row_when_value_cleared_name_already_empty(
    ...
    # Row should be removed, but if it was the last row,
    # a new empty row is appended
    # So the row count should remain the same (row deleted, new one appended)
    assert parameter_list_view.rowCount() == initial_row_count
